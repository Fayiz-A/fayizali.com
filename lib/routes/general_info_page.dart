...
import 'package:fayizali/widgets/GoBackButton.dart';
...
class GeneralInfoPage extends StatelessWidget {
  final List<Map<String, String>> informationList = [
    ...
    {
      'Address':
      'K-76/A, Abul Fazal Enclave, Jamia Nagar, Okhla, New Delhi, India'
    },
    {'Brother\'s Name': 'Isa Ali'}
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        body: Stack(children: [
          Background(),
          for (int index = 0; index < informationList.length; index++)
            CustomAnimatedBubble(
                index: index, bubbleDisplayInformation: informationList[index]),
          Padding(
...
class _BackgroundState extends State<Background> {
  ...
  void initState() {
    ...
    Timer.periodic(Duration(milliseconds: 1500),
            (timer) => colorBloc.add(RandomColorGeneratorEvent()));
  }
